...
function getFirstIpAddress(cidrStr, callback) {
  ...
  if (!cidr.isValid()) {
    ...
    callbackError = 'Error: Invalid CIDR passed to getFirstIpAddress.' + cidr;
  } else {
...
//   const ipv4Mapped = getIpv4MappedIpv6Address(firstIpAddress);
...
const ipSet = {
    ipv4: firstIpAddress || null,
    ipv6: getIpv4MappedIpv6Address(firstIpAddress) || null
}

  return callback(ipSet, callbackError);
}
...
function getIpv4MappedIpv6Address(ipv4) {
  ...
  if (numIpv4Segments === 4) {
    let validQuads = true;
    // Iterate over the IPv4 address parts afirstIpAddressnd verify each segment was a number between 0 and 255.
